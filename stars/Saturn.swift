...
struct Saturn: View {
    ...
    @State private var distance: Double = 1400000000
                        ...
                        ZStack{
                            ...
                            if back{
                                ZStack{
                                    ...
                                    Text("If you put all the planets in water, only Saturn would float!")
                                        .fontWeight(.bold)
                                        .font(.system(size: 24, design: .rounded))
                                        .frame(width: geometry.size.width * 0.55)
                                        .position(x: geometry.size.width * 0.52, y: geometry.size.height * 0.7)
                                    Button(action: {
                                        self.nextstar = true
                                        c.card14 = true
...
struct Saturn_previews: PreviewProvider{
    static var previews: some View{
        NavigationStack{
            Saturn().environmentObject(collect())
        }
    }
}
